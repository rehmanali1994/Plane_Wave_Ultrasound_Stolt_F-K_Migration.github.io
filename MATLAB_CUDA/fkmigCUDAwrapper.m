function [migSIG, param] = fkmigCUDAwrapper(SIG, param)
...
for idx = 1:Nframes
    % Grab Signals for a Single Plane-Wave Frame
    SIG_idx = SIG(:,:,idx)';
    
    % Perform Stolt's F-K Migration on Those Signals
    migSIG_idx = fkmigCUDA(SIG_idx, param.fs, param.pitch, param.TXangle(idx), param.c, param.t0);
    
    % Coherent Compounding of Different Plane-Wave Images
    migSIG = migSIG + (migSIG_idx'/Nframes)/(nt*nx); % divide by nt*nx because CUDA FFTs are unnormalized
end
